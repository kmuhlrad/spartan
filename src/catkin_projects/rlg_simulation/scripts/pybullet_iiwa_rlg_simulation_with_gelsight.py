...
kIiwaUrdf = "${SPARTAN_SOURCE_DIR}/models/iiwa/iiwa_description/iiwa14_schunk_gripper_with_gelsight.urdf"
...
class IiwaRlgSimulator():
    ...
    def ResetSimulation(self):
        ...
        if config["robot"]:
            ...
            self.gelsight_joint_id = pybullet.getNumJoints(self.kuka_id) - 1
    ...
    # Based on code from Achu Wilson (achuwilson@gmail.com)
    # as a part of the BlindGrasp project: http://achuwilson.github.io/projects/blindgrasp
    def GetGelsightRay(self, msg):
        if msg.data:
            gelsight_width = 0.024
            gelsight_height = 0.024
            num_h_pixel = 256
            num_v_pixel = 256

            pybullet.MAX_RAY_INTERSECTION_BATCH_SIZE = 300

            link_state = pybullet.getLinkState(self.kuka_id, self.gelsight_joint_id)
            ...
            transform[:3, :3] = -R
            transform[:3, 3] = np.array(gelsight_pos)
            transform[3, 3] = 1

            start_corner_x = -gelsight_width/2
            start_corner_y = -gelsight_height/2

            h_pixels = np.linspace(start_corner_x, start_corner_x + gelsight_width, num_h_pixel)
            ...
            for x in h_pixels:
                ...
                for y in v_pixels:
                    vcnt += 0.02
                    ray_from = np.dot(transform, np.array([x, y, 0, 1]))[:3]
            ...
            scaled_polygon_pcl = pcl2.create_cloud_xyz32(header, point_cloud_data)
            #publish the message
            self.gelsight_pcl_publisher.publish(scaled_polygon_pcl)
            ...
            for key in events.keys():
                if events[key] & pybullet.KEY_WAS_TRIGGERED:
                    if key == ord('q'):
                        ...
                        fake_message = std_msgs.msg.Bool
                        fake_message.data = True
                        self.GetGelsightRay(fake_message)
